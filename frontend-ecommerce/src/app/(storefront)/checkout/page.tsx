...
export default function CheckoutPage() {
    ...
    const { isAuthenticated, user, login } = useAuth(); // Integrate Auth
    ...
    // Auto-skip account step if logged in
    if (isAuthenticated && step === 'account') {
        setStep('shipping');
    }

    const handleNext = async () => {
        if (step === 'account') setStep('shipping');
    ...
    if (step === 'success') {
        return (
            <div className="max-w-2xl mx-auto py-32 px-6 text-center">
                ...
                {isGuest && (
                    <div className="mb-12 p-8 bg-amber-50 rounded-3xl border border-amber-100 max-w-lg mx-auto">
                        <h4 className="font-black text-amber-900 uppercase tracking-tight mb-2">Track Your Order</h4>
                        <p className="text-sm font-bold text-amber-800 mb-6">Since you checked out as a guest, use this unique link to track your shipment.</p>
                        <div className="flex gap-4">
                            <input readOnly value="https://adis-harvest.com/track/token/xyz-123" className="w-full px-4 py-3 rounded-xl bg-white border border-amber-200 text-xs font-mono text-amber-900" />
                            <button className="px-6 py-3 bg-amber-900 text-white rounded-xl font-bold text-xs uppercase hover:bg-black transition-colors">Copy</button>
                        </div>
                    </div>
                )}
    ...
    return (
        <div className="max-w-[1400px] mx-auto px-6 py-12 md:py-24">
            ...
            <div className="flex items-center justify-center mb-20 max-w-3xl mx-auto px-4">
                {[
                    { id: 'account', icon: Check, label: 'Account' },
                    ...
                    <div key={s.id} className="flex items-center flex-1 last:flex-none">
                        <div className="flex flex-col items-center relative gap-4">
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${step === s.id ? 'bg-slate-900 text-white scale-110 shadow-2xl shadow-slate-900/20' :
                                (['account', 'shipping', 'payment', 'review'].indexOf(step) > idx ? 'bg-emerald-600 text-white' : 'bg-slate-50 text-slate-300 border border-slate-200')
                                }`}>
                                {['account', 'shipping', 'payment', 'review'].indexOf(step) > idx ? <Check className="w-7 h-7" /> : <s.icon className="w-7 h-7" />}
                            </div>
                        ...
                        {idx < 3 && (
                            <div className={`flex-1 h-0.5 mx-4 md:mx-10 rounded-full transition-colors duration-500 ${['account', 'shipping', 'payment', 'review'].indexOf(step) > idx ? 'bg-emerald-600' : 'bg-slate-100'
                                }`} />
                        )}
                    </div>
                ))}
                    ...
                    {step === 'account' && (
                        <div className="space-y-12">
                            <div className="text-center md:text-left">
                                <h2 className="text-4xl font-black text-slate-900 uppercase tracking-tight">How would you like to check out?</h2>
                                <p className="text-slate-500 font-bold mt-4 text-lg">Create an account for faster checkout and order tracking, or continue as a guest.</p>
                            </div>

                            <div className="grid md:grid-cols-2 gap-8">
                                {/* Member Login */}
                                <div className="p-10 rounded-[3rem] bg-white border border-slate-100 shadow-xl shadow-slate-200/50 relative overflow-hidden group hover:border-emerald-500/30 transition-all">
                                    <div className="absolute top-0 right-0 w-64 h-64 bg-slate-50 rounded-full -translate-y-1/2 translate-x-1/2 group-hover:bg-emerald-50 transition-colors" />
                                    <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-8 relative">I have an account</h3>

                                    <form className="space-y-6 relative">
                                        <div className="space-y-3">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Email</label>
                                            <input type="email" placeholder="name@example.com" className="w-full px-6 py-5 rounded-3xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-emerald-500 outline-none font-bold text-slate-900" />
                                        </div>
                                        <div className="space-y-3">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Password</label>
                                            <input type="password" placeholder="••••••••" className="w-full px-6 py-5 rounded-3xl bg-slate-50 border border-slate-100 focus:bg-white focus:border-emerald-500 outline-none font-bold text-slate-900" />
                                        </div>
                                        <button className="w-full py-5 rounded-3xl bg-slate-900 text-white font-black hover:bg-emerald-600 transition-all shadow-xl shadow-slate-900/20">
                                            Sign In & Checkout
                                        </button>
                                    </form>
                                    <div className="mt-8 text-center relative">
                                        <Link href="/register" className="text-xs font-bold text-slate-400 hover:text-emerald-600 transition-colors uppercase tracking-widest">
                                            Create an account
                                        </Link>
                                    </div>
                                </div>
                                ...
                                {/* Guest Checkout */}
                                <div className="p-10 rounded-[3rem] bg-slate-50 border border-transparent hover:bg-white hover:border-slate-200 transition-all flex flex-col justify-between">
                                    <div>
                                        <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-3xl mb-8 shadow-sm">
                                            🚀
                                        </div>
                                        <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-4">Checkout as Guest</h3>
                                        <p className="text-sm font-bold text-slate-500 leading-relaxed mb-8">
                                            You don't need an account to place an order. You can register later to track your order history.
                                        </p>
                                    </div>
                                    <button
                                        onClick={handleGuestCheckout}
                                        className="w-full py-5 rounded-3xl border-2 border-slate-200 text-slate-900 font-black hover:border-slate-900 hover:bg-slate-900 hover:text-white transition-all uppercase tracking-widest text-xs"
                                    >
                                        Continue as Guest
                                    </button>
                                </div>
