            ...
            <aside className="w-64 bg-[#0F172A] text-white flex flex-col fixed h-full z-50">
                ...
                <nav className="flex-1 px-4 py-4 space-y-8 overflow-y-auto custom-scrollbar">
                    ...
                    <div>
                        <Link
                            href="/admin"
                            className={`flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg transition-colors ${pathname === '/admin' ? 'bg-emerald-500/10 text-emerald-400' : 'hover:bg-slate-800 text-slate-400 hover:text-white'}`}
                        >
                            <LayoutDashboard className={`w-4 h-4 ${pathname === '/admin' ? 'text-emerald-400' : ''}`} />
                            Dashboard
                        </Link>
                    </div>
                            ...
                            {group.items.map((item, idy) => {
                                ...
                                return (
                                    <Link
                                        key={idy}
                                        href={item.href}
                                        className={`flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg transition-all border ${isActive ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'text-slate-400 hover:text-white hover:bg-slate-800/50 border-transparent hover:border-slate-700/50'}`}
                                    >
