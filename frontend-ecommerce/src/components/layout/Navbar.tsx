...
export function Navbar() {
    ...
    return (
        <header
            className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 px-4 md:px-6 ${isScrolled ? 'py-2 md:py-4' : 'py-4 md:py-10'
                }`}
        >
            <div className={`max-w-[1400px] mx-auto transition-all duration-700 ${isScrolled
                ...
                <div className="flex items-center justify-between">
                    ...
                    <Link href="/" className="group flex items-center gap-3 md:gap-5 hover:opacity-100 transition-all duration-500">
                        <div className="relative shrink-0">
                            ...
                            <div className={`relative w-0 h-0 md:w-0 md:h-0 bg-gradient-to-br from-emerald-600 to-green-700 rounded-lg md:rounded-[1.2rem] flex items-center justify-center font-black text-7xl md:text-7xl shadow-xl group-hover:-rotate-[-50deg] transition-transform duration-700 border border-white/20 ${isScrolled ? 'text-green-800' : 'text-white'
                                }`}>
                                አ
                            </div>
                        </div>
                        <div className="flex flex-col -space-y-1 md:-space-y-2">
                            <span className={`text-3xl md:text-[3.5rem] font-black tracking-tighter uppercase transition-all duration-700 ${isScrolled ? 'text-green-800' : 'text-white'
                                }`}>ዲስ</span>
                            <span className={`text-[8px] md:text-[14px] font-black tracking-[0.3em] md:tracking-[0.5em] uppercase transition-all duration-700 ${isScrolled ? 'text-emerald-600' : 'text-emerald-300'
                                }`}>Harvest</span>
                        </div>
                    </Link>
                    ...
                    <nav className="hidden lg:flex items-center gap-16">
                        {navLinks.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`relative group text-[11px] font-black tracking-[0.4em] uppercase transition-all duration-700 ${isScrolled ? 'text-slate-700 hover:text-emerald-700' : 'text-white/80 hover:text-white'
                                    }`}
                            >
                    ...
                    <div className="flex items-center gap-2 sm:gap-6 md:gap-10">
                        ...
                        >
                            ...
                            <span className={`absolute -top-1 -right-1 w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center text-[8px] md:text-[10px] font-black border-2 ${isScrolled ? 'bg-amber-500 text-slate-950 border-white' : 'bg-emerald-500 text-white border-[#01110e]'
                                }`}>
                                {itemCount}
                            </span>
                        </Link>
                        ...
                        <button
                            ...
                            className={`lg:hidden p-2 md:p-4 rounded-full transition-all duration-700 ${isScrolled
